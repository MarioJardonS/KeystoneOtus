...
#    segment = 100
    ...
    for i in range(segment):
        ...
        metrics = Parallel(n_jobs=40)(delayed( CalculateMetricsTaxons) (copiesTaxonI[j-i-1], list(rows[j].values())) for j in range(i+1,segment) )
...
# generate copies for parallel processing
    ...
    for k in range(numElts):
        copiesTaxonI.append(rawTaxonI.copy())
    metrics = Parallel(n_jobs=40)(delayed( CalculateMetricsTaxons) (copiesTaxonI[j-i-1], list(rows[j].values())) for j in range(numElts) )
...
#            simmulationMetrics.append([spearman, brayCurtis ])
    # test variance of spearman on shallow sample
        shallowSimmulation = Parallel(n_jobs=40)(delayed( CalculateMetricsTaxons) (permutations[i][s], permutations[j][s]) for s in range(shallowSample) )
    # if variance is not zero continue simmulations
        if CheckShallowSimmulation(shallowSimmulation):
            ...
            simmulationMetrics = Parallel(n_jobs=40)(delayed( CalculateMetricsTaxons) (permutations[i][s], permutations[j][s]) for s in range(shallowSample, numPermutations) )
        # calculate pvalues
            ...
            for simm in simmulationMetrics:
                sp_1 = simm[0]
                bc_1 = simm[1]
                simmulationsSpearman.append(sp_1)
                simmulationsBrayCurtis.append(bc_1)
